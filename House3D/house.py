...
import pickle
import time
from House3D.build_accurate_map import build_collision_map
...
class House(object):
    ...
    def __init__(self, JsonFile, ObjFile, MetaDataFile,
        ...
        Args:
            ...
            CacheConnMap (str, optional): if CacheConnMap is not None, pickle the connMap data
            GenRoomTypeMap (bool, optional): if turned on, generate the room type map for each location
            GenIndoorArea (bool, optional): if turned on, generate the indoor movable area
            DebugInfoOn (bool, optional): store additional debugging information when this option is on
            GridDet (float, optional): grid cell size of the 2d map for collision check
        ...
        self.house_dir = os.path.dirname(JsonFile)
        ...
        self.L_lo = np.array([_L_lo[0], _L_lo[2]])
        # make boundary value divisible by GridDet
        self.L_lo = np.floor(self.L_lo / GridDet) * GridDet
        self.L_max_coor = _L_hi = np.array(level['bbox']['max'])
        self.L_hi = np.array([_L_hi[0], _L_hi[2]])
        self.L_hi = np.ceil(self.L_hi / GridDet) * GridDet
        self.L_det = self.L_hi - self.L_lo
        self.grid_det = GridDet
        self.n_row = np.floor(self.L_det / self.grid_det).astype(int)
        ...
        if _IgnoreSmallHouse and (
                (len(self.all_desired_roomTypes) < 2) or ('kitchen' not in self.all_desired_roomTypes)):
            self.all_desired_roomTypes = []
            return
        ...
        self.obsMap = np.ones((self.n_row[0] + 1, self.n_row[1] + 1), dtype=np.uint8)  # a small int is enough
        if self._debugMap is not None:
            self._debugMap = np.ones((self.n_row[0] + 1, self.n_row[1] + 1), dtype=np.float)
        self.genObstacleMap()
        if DebugMessages == True:
            print('  --> Done! Elapsed = %.2fs' % (time.time() - ts))
        ...
        if DebugMessages == True:
            ...
            ts = time.time()
        self.moveMap = np.zeros((self.n_row[0] + 1, self.n_row[1] + 1), dtype=np.uint8)  # initially not movable
        self.genMovableMap(ApproximateMovableMap)
        if DebugMessages == True:
            print('  --> Done! Elapsed = %.2fs' % (time.time() - ts))

        self.connMap = self.moveMap
        conn_x, conn_y = np.where(self.connMap == 1)
        connectedCoors = np.stack((conn_x, conn_y)).T
        self.connectedCoors = connectedCoors
        ...
        if GenIndoorArea:
            if DebugMessages == True:
                ts = time.time()
                print('Generate Indoor Area Map ...')
            self.genIndoorConnMap()
            indoor_conn_x, indoor_conn_y = np.where(self.indoorConnMap == 1)
            indoorConnectedCoors = np.stack((indoor_conn_x, indoor_conn_y)).T
            self.indoorConnectedCoors = indoorConnectedCoors
        ...
        if GenRoomTypeMap:
            ...
            self.roomTypeMap = np.zeros((self.n_row[0] + 1, self.n_row[1] + 1),
                                        dtype=np.uint16)
    ...
    def _generate_room_type_map(self):
        ...
        for x in range(self.n_row[0] + 1):
            for y in range(self.n_row[1] + 1):
    ...
    def _find_components(self, x1, y1, x2, y2, dirs=None, return_largest=False, return_open=False):
        ...
        if return_open:
            if len(open_comps) == 0:
                print(
                    'WARNING!!!! [House] <find components in Target Room [%s]> No Open Components Found!!!! Return Largest Instead!!!!' % self.targetRoomTp)
                return_largest = True
            else:
    ...
    # """
    #
    # def setTargetRoom(self, targetRoomTp='kitchen'):
    #     if self.CacheConnMap is not None:
    #         split_filename = os.path.splitext(self.CacheConnMap)
    #         file_to_save = '.'.join((split_filename[0] + '_%s' % targetRoomTp,
    #                                  split_filename[1][1:]))
    ...
    #     elif self.CacheConnMap is not None and os.path.exists(file_to_save):
    #         with open(file_to_save, 'rb') as f:
    #             self.connMap, self.connectedCoors, self.inroomDist, self.maxConnDist = pickle.load(f)
    #         self.connMapDict[targetRoomTp] = (self.connMap, self.connectedCoors,
    #                                           self.inroomDist, self.maxConnDist)
    #         return True
    ...
    #     ##########
    #
    #     print('[House] Caching New ConnMap for Target <{}>! (total {} rooms involved)'.format(targetRoomTp,
    #                                                                                           len(targetRooms)))
    #     self.connMap = connMap = np.ones((self.n_row[0] + 1, self.n_row[1] + 1), dtype=np.int32) * -1
    #     self.inroomDist = inroomDist = np.ones((self.n_row[0] + 1, self.n_row[1] + 1), dtype=np.float32) * -1
    #     dirs = [[0, 1], [1, 0], [-1, 0], [0, -1]]
    #     que = []
    #     for flag_find_open_components in [True, False]:
    #         if not flag_find_open_components:
    #             print(
    #                 'WARINING!!!! [House] No Space Found for Room Type {}! Now search even for closed region!!!'.format(
    #                     targetRoomTp))
    ...
    #             x1, y1, x2, y2 = self.rescale(_x1, _y1, _x2, _y2)
    #             curr_components = self._find_components(x1, y1, x2, y2, dirs=dirs,
    #                                                     return_open=flag_find_open_components)  # find all the open components
    ...
    #     if self.CacheConnMap is not None:
    #         print('saving coonMap for %s to %s' % (targetRoomTp, file_to_save))
    #         with open(file_to_save, 'wb') as f:
    #             pickle.dump(self.connMapDict[targetRoomTp], f,
    ...
    def genObstacleMap(self):
        if self.render_door:
            obs_map_file = os.path.join(self.house_dir, 'obs_map_%s_%s.png' % ('door', str(self.grid_det)))
        else:
            obs_map_file = os.path.join(self.house_dir, 'obs_map_%s.png' % str(self.grid_det))
        if os.path.exists(obs_map_file):
            obsMap = cv2.imread(obs_map_file, cv2.IMREAD_GRAYSCALE) / 255
            obsMap = (1 - obsMap).astype(np.uint8)
        else:
            obsMap = 1 - build_collision_map(obj_file=self.objFile,
                                             json_file=self.jsonFile,
                                             save_file=obs_map_file,
                                             lower_limit=10,
                                             upper_limit=180,
                                             resolution=self.grid_det,
                                             N_SAMPLES_PER_FACE=200,
                                             LARGEST_CC=True)
        self.obsMap = obsMap

    def genIndoorConnMap(self):
        if self.render_door:
            indoor_conn_map_file = os.path.join(self.house_dir, 'indoor_conn_%s_%s.png' % ('door', str(self.grid_det)))
        else:
            indoor_conn_map_file = os.path.join(self.house_dir, 'indoor_conn_%s.png' % str(self.grid_det))
        if os.path.exists(indoor_conn_map_file):
            indoorConnMap = cv2.imread(indoor_conn_map_file, cv2.IMREAD_GRAYSCALE) / 255
            indoorConnMap = indoorConnMap.astype(np.uint8)
        else:
            self.roomTypeMap = np.zeros((self.n_row[0] + 1, self.n_row[1] + 1),
                                        dtype=np.uint16)
            self._generate_room_type_map()
            indoorConnMap = self.roomTypeMap.copy()
            indoorConnMap[self.roomTypeMap > 1] = 1  # 1 is outdoor, 0 is nothing, >1 indoor
            indoorConnMap[self.roomTypeMap <= 1] = 0
            cv2.imwrite(indoor_conn_map_file, indoorConnMap.astype(np.uint8) * 255)
        self.indoorConnMap = indoorConnMap

    def genMovableMap(self, approximate=False):
        if self.render_door:
            move_map_file = os.path.join(self.house_dir, 'move_map_%s_%s.png' % ('door', str(self.grid_det)))
        else:
            move_map_file = os.path.join(self.house_dir, 'move_map_%s.png' % str(self.grid_det))
        if os.path.exists(move_map_file):
            moveMap = cv2.imread(move_map_file, cv2.IMREAD_GRAYSCALE) / 255
            moveMap = moveMap.astype(np.uint8)
            ...
            cv2.imwrite(move_map_file,
                        self.moveMap.astype(np.uint8) * 255)
    ...
    def _adjustApproximateRobotMoveMap(self):
        ...
        robotGridSize = int(np.rint(self.robotRad * 2 / self.grid_det))
        if robotGridSize > 1:
    ...
    def _getRegionsOfInterest(self):
        ...
        """
        return [(0, 0, self.n_row[0] + 1, self.n_row[1] + 1)]
    ...
    def inside(self, x, y):
        return x >= 0 and y >= 0 and x <= self.n_row[0] and y <= self.n_row[1]
    ...
    def rescale(self, x1, y1, x2, y2, n_row=None):
        if n_row is None:
            n_row = self.n_row
            grid_det = [self.grid_det] * 2
        else:
            grid_det = self.L_det / n_row
        tiny = 1e-9
        tx1 = np.floor((x1 - self.L_lo[0]) / grid_det[0] + tiny)
        ty1 = np.floor((y1 - self.L_lo[1]) / grid_det[1] + tiny)
        tx2 = np.floor((x2 - self.L_lo[0]) / grid_det[0] + tiny)
        ty2 = np.floor((y2 - self.L_lo[1]) / grid_det[1] + tiny)
    ...
    def to_grid(self, x, y, n_row=None):
        ...
        if n_row is None:
            n_row = self.n_row
            grid_det = self.grid_det
        else:
            grid_det = self.L_det / n_row
            try:
                assert np.abs(grid_det[0] - grid_det[1]) < 0.0001
            except AssertionError:
                print('n_row:', n_row)
                print('L_det:', self.L_det)
                import sys
                sys.exit()
            grid_det = grid_det[0]
        tiny = 1e-9
        tx = np.floor((x - self.L_lo[0]) / grid_det + tiny)
        ty = np.floor((y - self.L_lo[1]) / grid_det + tiny)
    ...
    def to_coor(self, x, y, shft=False):
        ...
        """
        tx, ty = x * self.grid_det + self.L_lo[0], y * self.grid_det + self.L_lo[1]
        if shft:
    ...
    def _check_grid_occupy(self, cx, cy, gx, gy):
        for x in range(gx, gx + 2):
            for y in range(gy, gy + 2):
                rx, ry = x * self.grid_det + self.L_lo[0], y * self.grid_det + self.L_lo[1]
                if (rx - cx) ** 2 + (ry - cy) ** 2 <= self.robotRad ** 2:
                    return True
        return False

    """
    suppose the robot stands at continuous coordinate (cx, cy), check whether it will touch any obstacles,
    return True if it won't touch any obstable, False otherwise
    """
    ...
    def isConnect(self, gx, gy):
        return (self.inside(gx, gy)) and (self.connMap[gx, gy] > 0)
